...
from pygovpub.core.db_fixtures import (
    create_versioned_records,
    create_relationship_test_data,
    create_legislative_text_corpus,
    create_bill_hierarchy,
...
class TestVersionedRecords:
    ...
    def test_create_versioned_records(self, setup_test_tables):
        ...
        # Set up mock return value for unit tests
        with mock.patch('pygovpub.core.db_fixtures.create_versioned_records') as mock_create:
            records = [
            ...
            mock_create.return_value = records
            
            # Call function
            created_records = mock_create(
...
class TestRelationshipFixtures:
    ...
    def test_create_relationship_test_data(self, setup_test_tables):
        ...
        # Setup mock return
        with mock.patch('pygovpub.core.db_fixtures.create_relationship_test_data') as mock_create:
            relationship_data = {
                "authors": [
            ...
            mock_create.return_value = relationship_data
...
class TestLegislativeFixtures:
    ...
    def test_create_legislative_text_corpus(self):
        ...
        # Setup mock return
        with mock.patch('pygovpub.core.db_fixtures.create_legislative_text_corpus') as mock_create:
            sample_corpus = [
                {
                    "id": 1,
            ...
            mock_create.return_value = sample_corpus
...
class TestBillHierarchyFixtures:
    ...
    def test_create_bill_hierarchy(self):
        ...
        # Setup mock return
        with mock.patch('pygovpub.core.db_fixtures.create_bill_hierarchy') as mock_create:
            hierarchy = {
                "bill": {
                    "id": 1,
                    "congress": 117,
            ...
            mock_create.return_value = hierarchy
            
            # Call function
            bill_data = mock_create(
                congress=117,
                bill_type="HR",
                bill_number=1234,
                version_count=3,
...
class TestDatabaseSessionFixtures:
    ...
    def test_create_read_only_session(self):
        ...
        # Create mock session
        with mock.patch('pygovpub.core.db_fixtures.create_read_only_session') as mock_create:
    ...
    def test_with_multi_database(self):
        ...
        with mock.patch('pygovpub.core.db_fixtures.with_multi_database') as mock_multi_db:
            mock_sessions = {"primary": mock.MagicMock(), "replica": mock.MagicMock()}
            mock_context = mock.MagicMock()
            mock_context.__enter__.return_value = mock_sessions
            mock_multi_db.return_value = mock_context
            
            # Use context manager
            with mock_multi_db(["primary", "replica"]) as sessions:
...
def test_cleanup_fixtures():
    ...
    with mock.patch('pygovpub.core.db_fixtures.cleanup_fixtures') as mock_cleanup:
        # Simply test that the function doesn't throw an error
        # with minimal tables
        mock_cleanup(tables=["test_authors", "test_documents"])
        
        # Verify mock was called with correct parameters
        mock_cleanup.assert_called_once_with(tables=["test_authors", "test_documents"])